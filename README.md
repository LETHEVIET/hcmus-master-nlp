...
Model/công cụ có sẵn chỉ được dùng để hỗ trợ tách câu hoặc gán nhãn NER tự
động. Pipeline hiện kết hợp luật hình thức, seed gazetteer và tên người từ
CBDB; review thủ công là bước tùy chọn nếu cần xây dựng gold corpus.
...
Để chạy pipeline và lấy các artifact đã commit:

- Git và Git LFS;
- [`uv`](https://docs.astral.sh/uv/);
- Python 3.11 (được `uv` quản lý theo `.python-version`);
- `curl`, `unzip` và `sha256sum` để lấy đúng bản CBDB;
- XeLaTeX cùng các font DejaVu, Noto Sans Mono và Droid Sans Fallback nếu muốn
  biên dịch lại report PDF.

Cài `uv`, sau đó đồng bộ đúng dependency từ lock file:
...
## Tái tạo chính xác kết quả đã nộp

Các số liệu và output hiện tại sử dụng CBDB release `2026.07.04`. CBDB được
phân phối theo CC BY-NC-SA 4.0; hãy đọc và chấp nhận điều kiện tại
[`docs/external-sources.md`](docs/external-sources.md) trước khi tải.

Từ thư mục gốc repository, tải và kiểm tra đúng SQLite nguồn:

```bash
mkdir -p data/external
curl -L \
  https://huggingface.co/datasets/cbdb/cbdb-sqlite/resolve/main/history/cbdb_202607/cbdb_20260704.zip \
  -o data/external/cbdb_20260704.zip
unzip -jo data/external/cbdb_20260704.zip \
  cbdb_20260704.sqlite3 -d data/external
printf '%s  %s\n' \
  6b18f1e7f90a823d8dde5ecbd8a9eac70f092fe643efb8dca193986949a913df \
  data/external/cbdb_20260704.sqlite3 | sha256sum -c -
```

Tạo lại KB cache và toàn bộ corpus/output:
...
uv run python scripts/annotate_corpus.py \
  --cbdb-short-names exclude \
  --cbdb-period-policy strict \
  --cbdb-max-ambiguity 10
...
uv run python scripts/visualize_samples.py \
  --input build/corpus_preannotated.jsonl \
  --output build/annotation_samples.html \
  --limit 24
uv run python scripts/export_submission.py --mode draft
uv run python scripts/compliance_check.py \
  --submission build/submission \
  --source-corpus build/corpus_preannotated.jsonl \
  --mode draft
```

Biên dịch report (hai lượt để resolve cross-reference/citation):

```bash
mkdir -p build/report
xelatex -interaction=nonstopmode -halt-on-error \
  -output-directory=build/report docs/report.tex
xelatex -interaction=nonstopmode -halt-on-error \
  -output-directory=build/report docs/report.tex
```
...
| Câu | 271.471 |
| Entity | 208.558 |
| Entity `PERSON` từ CBDB | 18.150 |
| Compliance draft | 0 fatal, 0 warning |

Các checksum quan trọng:

```text
build/corpus.jsonl
  b168b308ddd7217d9fb356b4c93531268f0bcb41e91adcf0389b0323bed5bab0
build/corpus_preannotated.jsonl
  ff95ce1375b09e1cb7363cd5277aeb6c4f051bb7a2fb81bc10276c1e62163dbc
build/kb/cbdb.sqlite
  97abb9454ca6a36acffdcfa912dfd1633a89ffde6f879a3ecd0211775c260c0a
```

Cache CBDB không được commit do điều kiện phân phối; manifest của cache được
commit để lưu provenance. Khi `build/kb/cbdb.sqlite` tồn tại và hash khớp
manifest, `annotate_corpus.py` tự động dùng CBDB. Dùng `--no-cbdb` chỉ khi muốn
chạy baseline không tái tạo số liệu trong bảng trên.
...
dùng pipeline source hiện tại (mặc định regex + seed + CBDB nếu có cache). Vì hai
...
NER hiện tại là gán nhãn tự động dựa trên luật và từ điển với các nhãn:
...
Tên người (`PERSON`) được tra từ CBDB và mang theo mã nhân vật để truy vết.
Các tên trùng nhiều nhân vật được ghi trong `linking_candidates`; từ điển xác
định span và loại thực thể, không khẳng định đã định danh đúng một cá nhân.
...
## Review thủ công tùy chọn

1. Review các câu có `review_status: "needs_review"`.
2. Sửa hoặc xóa entity heuristic bị sai.
3. Bổ sung entity bị bỏ sót hoặc phân giải tên người trùng nhau.
4. Đổi trạng thái các record đã kiểm tra sang `checked`.
5. Chỉ sau bước này mới gọi dữ liệu là gold/curated corpus; bản gán nhãn tự
   động vẫn có thể nộp như một dataset tự động nếu đề bài không yêu cầu gold.
...
- 208.558 entity candidates, trong đó 18.150 nhãn `PERSON` từ CBDB.
- Trạng thái: dataset được sinh tự động; các cờ review chỉ phục vụ kiểm toán.

Đây là corpus dẫn xuất tự động, không tuyên bố là gold corpus đã được con người
